...
class CalendarAdapter(fragmentActivity: FragmentActivity):FragmentStateAdapter(fragmentActivity) {
    private var now: Long = DateTime().withDayOfMonth(1).withTimeAtStartOfDay().millis
    ...
    override fun getItemId(position: Int): Long = DateTime(now).plusMonths(position - START_POSITION).millis
